...
def show_engineering_loader(
  ...
  with st.status(f"🏗️ {text}", expanded=True) as status:
    ...
    status.update(
        label="✅ עיבוד S.A.Q הסתיים בהצלחה!", state="complete", expanded=False
    )
...
def run_ai_verification_workflow(raw_plan, results_list, session_key_verified):
  ...
  if yellow_items and not is_done_verifying:
    st.markdown("---")
    st.info(
        "🔍 **אישור סמלים בספק ולמידת AI (מוצגים"
        f" {len(yellow_items)} פריטים ראשונים לבדיקה):**"
    )
    with st.expander(
        "לחץ כאן לאישור/דחיית סמלים (ה-AI לומד לפעמים הבאות)", expanded=True
    ):
...
with st.sidebar:
  ...
  if st.session_state["current_discipline"] == "📐 ריצוף וחיפוי":
    tile_h = st.number_input(
        "גובה חיפוי קירות בחדרים רטובים (מטר):",
        min_value=1.5,
        max_value=3.5,
        value=2.40,
        step=0.10,
...
if st.session_state.get("show_master_export", False):
  ...
  for d_name in disciplines_list:
    ...
    with st.expander(
        f"📋 {d_name} ({len(d_rows)} שורות שנשמרו)", expanded=True
    ):
      if d_rows:

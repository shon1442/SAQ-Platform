...
# ========================================================
# מנוע עיצוב מרכזי - חסין דליפות, מצב כהה/בהיר וללא אימוג'ים
# ========================================================
...
if st.session_state["dark_mode"]:
    css_code += ".stApp { background-color: #121212 !important; color: #ffffff !important; }\n"
    css_code += "h1, h2, h3, h4, p, span, label { color: #ffffff !important; }\n"
    css_code += "div[data-testid='stAppViewContainer'] { background-color: #121212 !important; }\n"
...
if app_mode in ["Tenant_CO", "שינויי דיירים"] and not st.session_state["dark_mode"]:
...
elif app_mode in ["Renovation", "קבלני שיפוצים"] and not st.session_state["dark_mode"]:
...
def safe_render_table(rows, is_us=False):
    cols = (["No.", "Symbol", "Item Description", "Approved Qty", "Unit"]
            if is_us else ["מס'", "תמונת סמל", "תיאור הפריט", "כמות מאושרת", "יחידת מידה"])
    if not rows:
    ...
    st.dataframe(df, column_config={cols[1]: st.column_config.ImageColumn("Engineering Symbol" if is_us else "סמל / תרשים הנדסי", width="small")})

def validate_drawing_discipline(img, expected_disc, is_us=False):
    if img is None: return False, ("Invalid file." if is_us else "קובץ לא קריא.")
    try:
        h, w = img.shape[:2]
        max_dim = 800
        if max(h, w) > max_dim:
        ...
        if expected_disc == "elec" and circular_symbols < 3: return False, ("Engineering Alert: Drawing lacks electrical symbols." if is_us else "זיהוי אוטומטי: לא נמצאו סמלי חשמל ומאור.")
        elif expected_disc == "cons" and lines < 5: return False, ("Engineering Alert: Drawing lacks continuous walls/partitions." if is_us else "זיהוי אוטומטי: לא נמצאו מספיק קירות או מחיצות.")
        elif expected_disc == "plum" and plumbing_fixtures < 1 and circular_symbols < 2: return False, ("Engineering Alert: No plumbing fixtures detected." if is_us else "זיהוי אוטומטי: לא נמצאו כלים סניטריים.")
        elif expected_disc == "hvac" and circular_symbols < 2: return False, ("Engineering Alert: Drawing lacks HVAC symbols." if is_us else "זיהוי אוטומטי: לא נמצאו סמלי מיזוג אוויר.")
        elif expected_disc == "kitc" and lines < 2: return False, ("Engineering Alert: Drawing lacks cabinetry outlines." if is_us else "זיהוי אוטומטי: לא נמצאו קווי נגרות.")
        return True, ""
...
def show_engineering_loader(text="S.A. Quantities AI is processing data...", is_us=False):
    ...
    for i in range(steps):
        ...
        if percent < 40: status_box.markdown(f"**[Active Construction Site]** Scanning: {text}" if is_us else f"**[אתר בניה פעיל]** סורק שרטוטים: {text}")
        elif percent < 80: status_box.markdown("**[AI Engine]** Running algorithms..." if is_us else "**[מנוע חישוב AI]** מפעיל חישובים הנדסיים...")
        else: status_box.markdown("**[Final Reports]** Compiling quantities..." if is_us else "**[דוחות סופיים]** ממצה כמויות...")
    progress_bar.empty()
    status_box.success("Takeoff completed successfully!" if is_us else "פענוח האתר הסתיים בהצלחה!")

# ========================================================
# אנימציית פתיחה - Time-lapse מתמונה אחת
# ========================================================
if "splash_shown" not in st.session_state:
    st.session_state["splash_shown"] = True
    splash_css = ""
    splash_css += "<style>\n"
    splash_css += ".fullscreen-splash { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #0f172a; z-index: 999999; display: flex; flex-direction: column; justify-content: center; align-items: center; overflow: hidden; animation: hideSplash 4s forwards ease-in-out; }\n"
    splash_css += "@keyframes hideSplash { 0% { opacity: 1; } 85% { opacity: 1; } 100% { opacity: 0; visibility: hidden; pointer-events: none; z-index: -10; display: none; } }\n"
    splash_css += ".timelapse-container { width: 90vw; max-width: 1200px; height: 60vh; background-image: url('Gemini_Generated_Image_3zojl43zojl43zoj.jpeg'); background-size: cover; background-position: 0% center; border-radius: 12px; box-shadow: 0 15px 40px rgba(0,0,0,0.5); animation: buildProgress 3.5s ease-in-out forwards; }\n"
    splash_css += "@keyframes buildProgress { 0% { background-position: 0% center; } 100% { background-position: 100% center; } }\n"
    splash_css += ".splash-text { margin-top: 30px; font-size: 42px; font-weight: 700; color: #f8fafc; font-family: 'Segoe UI', Arial, sans-serif; letter-spacing: 2px; text-shadow: 0 2px 10px rgba(0,0,0,0.5); opacity: 0; animation: textFade 1.5s 0.5s forwards ease-in-out; }\n"
    splash_css += "@keyframes textFade { to { opacity: 1; transform: translateY(-10px); } }\n"
    splash_css += "</style>\n"
    
    splash_html = "<div class='fullscreen-splash' translate='no'>\n"
    splash_html += "<div class='timelapse-container'></div>\n"
    splash_html += "<div class='splash-text'>S.A.Q. מחשבים את העתיד</div>\n"
    splash_html += "</div>\n"
...
def get_pricing_item_cost(desc, unit, is_us=False, is_after_exec=False):
    multiplier = 1.35 if is_after_exec else 1.0
    if is_us:
        base_price = 45
        ...
        if "Cabinet" in desc or "Countertop" in desc: base_price = 250
        ...
        elif "יח'" in unit or "כלי" in desc or "נקודת" in desc: unit_price = 450
        ...
        if "צנרת" in desc or "כבל" in desc or "חציבת" in desc or "ביטון" in desc: unit_price = 85
        if "ארונות" in desc or "שיש" in desc or "מטבח" in desc: unit_price = 1200
        return unit_price * multiplier

def render_pricing_widget(boq_rows, discipline_name, is_us=False, is_after_exec=False):
    currency_sign = "$" if is_us else "₪"
    pricing_title = "RSMeans Pricing (USA - $)" if is_us else "מחירון דקל (ישראל - ₪)"
    expander_lbl = (f"Estimated Price & Costing via {pricing_title} ({discipline_name})" if is_us else f"הצג הערכת מחיר ותמחור משוער לפי {pricing_title} ({discipline_name})")
    with st.expander(expander_lbl, expanded=False):
        st.info(f"Pricing is automatically calculated based on active regional database ({pricing_title}):" if is_us else f"התמחור מחושב אוטומטית לפי מחירי {pricing_title} מעודכנים לענף:")
        total_est_price = 0
        pricing_data = []
        for idx, row in enumerate(boq_rows):
            ...
            if is_us:
                ...
                elif "יח'" in unit: unit = "Units"
            unit_price = get_pricing_item_cost(desc, unit, is_us, is_after_exec)
            if st.session_state.get("include_vat", True): unit_price = unit_price * (1.085 if is_us else 1.17)
        ...
        st.success(f"**Estimated Total Cost for these items: {total_est_price:,.2f} {currency_sign}**" if is_us else f"**עלות כוללת מוערכת לפריטים אלו: {total_est_price:,.2f} ₪**")
        ...
        if (w * 0.1 * h * 0.1) < area < (w * 0.4 * h * 0.4):
            ...
            if M["m00"] > 0:
                ...
                if cX < w * 0.15 or cX > w * 0.85:
                    ...
                    msg = (f"Critical Engineering Alert: Potential breach at concrete column (X:{cX}, Y:{cY})" if is_us else f"התראה הנדסית קריטית (מעטפת/ממדי): זוהתה פגיעה פוטנציאלית בעמוד קונסטרוקטיבי בנקודה (X:{cX}, Y:{cY})")
                    alerts.append(msg)
...
def extract_interior_walls_clean(plan_img, px_per_meter=125.0):
    ...
    for c in contours:
        if cv2.contourArea(c) >= min_wall_area: cv2.drawContours(clean_interior_mask, [c], -1, 255, -1)
...
def detect_sanitary_fixtures_and_points(plan_img, px_per_meter=125.0):
    ...
    for c in contours:
        ...
        if (1.2 <= max_dim <= 2.2) and (0.6 <= min_dim <= 1.0) and area > 900:
            ...
            f_type = "Gas Point" if 250 < area < 400 else "Toilet"
            fixtures.append({"type": f_type, "center": (x + w // 2, y + h // 2), "bbox": (x, y, w, h), "crop": plan_img[max(0, y - 5) : min(plan_img.shape[0], y + h + 5), max(0, x - 5) : min(plan_img.shape[1], x + w + 5)], "status": "Yellow" if area < 1000 else "Green", "score": 0.85})
...
def compare_plumbing_delta_accurate(plan_std, plan_exec, px_per_meter=125.0):
    ...
    relocations = []; added = []; b_matched = set()
    for f_a in fix_std:
        ca = f_a["center"]
        best_dist = 999999
        best_idx_b = -1
        ...
        if best_idx_b != -1:
            ...
            relocations.append({"type": f_b["type"], "distance_m": dist_m, "from": ca, "to": f_b["center"], "radius_exceeded": dist_m > 1.5})
            cv2.arrowedLine(disp_exec, ca, f_b["center"], (0, 140, 255), 3, tipLength=0.20)
    for idx_b, f_b in enumerate(fix_exec):
        if idx_b not in b_matched: added.append(f_b)
...
def compare_hvac_delta_accurate(plan_std, plan_exec, px_per_meter=125.0):
    ...
    relocations = []; added = []; b_matched = set(); extra_pipe_m = 0.0
    for f_a in fix_std:
        ca = f_a["center"]
        best_dist = 999999
        best_idx_b = -1
...
def calc_kitchen_cabinetry(plan_img, px_per_meter=125.0):
    gray = cv2.cvtColor(plan_img, cv2.COLOR_BGR2GRAY)
    _, thresh = cv2.threshold(gray, 220, 255, cv2.THRESH_BINARY_INV)
    linear_pixels = cv2.countNonZero(thresh) * 0.15 
    linear_meters = round(linear_pixels / float(px_per_meter), 2)
    return linear_meters, plan_img
...
def run_ai_verification_workflow(raw_plan, results_list, session_key_verified, is_us=False):
    ...
    if yellow_items and not is_done_verifying:
        st.markdown("---")
        st.markdown("### Active Learning & AI Verification" if is_us else "### מנגנון למידה אקטיבית של S.A.Q AI")
        st.warning("Please complete symbol verification to view final takeoff report." if is_us else "אנא השלם את אימות הסמלים במערכת הלמידה כדי לצפות בדוח הכמויות הסופי.")
        with st.expander("Symbol Verification Control Center" if is_us else "מרכז בקרת סמלים - הדרכת AI (לחץ לפתיחה)", expanded=True):
            ...
            for y_i, (s_idx, m_idx, item, m) in enumerate(yellow_items):
                with cols[y_i % len(cols)]:
                    ...
                    crop_zoom = np.zeros((100, 100, 3), dtype=np.uint8) if y2 <= y1 or x2 <= x1 else raw_plan[y1:y2, x1:x2].copy()
                    ...
                    choice = st.radio("Inspector Decision:" if is_us else "החלטת מפקח:", ["Approve", "Reject"] if is_us else ["אשר", "דחה"], key=f"verify_choice_{session_key_verified}_{s_idx}_{m_idx}_{y_i}", horizontal=True)
            ...
            if st.button("Finished Verification - Lock Quantities & Proceed" if is_us else "סיימתי את הבקרה - נעל כמויות והמשך", key=f"btn_lock_{session_key_verified}"):
    ...
    for s_idx, item in enumerate(results_list):
        ...
        for m_idx, m in enumerate(item["matches"]):
            ...
            if is_green or user_dec == "Approved":
                ...
                cv2.line(disp_plan, (x, y), (x + w, y + h), (0, 0, 255), 2); cv2.line(disp_plan, (x + w, y), (x, y + h), (0, 0, 255), 2)
        item["confirmed_count"] = confirmed_count
        if confirmed_count > 0:
            rows.append({"מס'": item["index"], "תמונת סמל": item["image_uri"], "image_uri": item["image_uri"], "תיאור הפריט": f"Verified Symbol #{item['index']}" if is_us else f"סמל מנותח #{item['index']}", "כמות מאושרת": confirmed_count, "יחידת מידה": "Units" if is_us else "יח'"})
...
def generate_master_export_html(project_boq, title="דוח כתב כמויות מאוחד לפרויקט", mode_label="שינויי דיירים", is_us=False, include_pricing=True):
    ...
    currency_sign = "$" if is_us else "₪"
    tax_label = "Local Tax (8.5%)" if is_us else "מע\"מ"
    tax_rate = 0.085 if is_us else 0.17
    ...
    if include_pricing:
        for disc_name, rows in project_boq.items():
            for r in rows:
                ...
                is_after_exec = st.session_state.get(f"{disc_name}_after_exec", False)
                grand_total_pricing += qty * get_pricing_item_cost(desc, unit, is_us, is_after_exec)
        tax_amount = grand_total_pricing * tax_rate if st.session_state.get("include_vat", True) else 0
    ...
    if include_pricing: headers.extend(["Unit Price", "Total"] if is_us else ["מחיר יחידה", 'סה"כ'])
    html = f"""
    <html dir="{dir_attr}">
    ...
    <div class="header-box"><div><h2>S.A. Quantities AI (S.A.Q)</h2><p><b>{title}</b> | {"Model:" if is_us else "מודל:"} <b>{mode_label}</b></p></div><div>{logo_html}</div></div>
    """
    disp_names = {"elec": "Electrical & Lighting" if is_us else "חשמל ומאור", "cons": "Construction (Walls)" if is_us else "בניה (מחיצות ומעטפת)", "plum": "Plumbing & Gas" if is_us else "אינסטלציה וגז", "tile": "Flooring & Tiling" if is_us else "ריצוף וחיפוי", "hvac": "HVAC & Infrastructure" if is_us else "מיזוג אוויר ותשתיות", "kitc": "Kitchen & Cabinetry" if is_us else "מטבח ונגרות"}
    for disc_key, rows in project_boq.items():
        ...
        if not rows: html += f"<tr><td colspan='{len(headers)}'>{'No quantities recorded' if is_us else 'לא נרשמו כמויות'}</td></tr>"
        else:
            for r in rows:
                desc = r.get("תיאור הפריט", "")
                ...
                if include_pricing:
                    is_after_exec = st.session_state.get(f"{disc_key}_after_exec", False)
                    u_price = get_pricing_item_cost(desc, u_disp, is_us, is_after_exec)
                    if st.session_state.get("include_vat", True): u_price = u_price * (1.085 if is_us else 1.17)
                    tot_price = q_disp * u_price
                    row_html += f"<td>{u_price:,.2f} {currency_sign}</td><td>{tot_price:,.2f} {currency_sign}</td>"
                html += row_html + "</tr>"
        html += "</table>"
    if include_pricing:
        html += f'<div class="total-box"><h3>{"Total Project Pricing" if is_us else "סיכום תמחור כללי"}</h3><p><b>{"Total (Excl. Tax):" if is_us else "סה\"כ לפני מיסים:"}</b> {grand_total_pricing:,.2f} {currency_sign}</p>'
        if st.session_state.get("include_vat", True): html += f"<p><b>{tax_label}:</b> {tax_amount:,.2f} {currency_sign}</p>"
        html += f'<hr style="border: 0; border-top: 1px solid rgba(255,255,255,0.3); margin: 10px 0;"><p style="font-size: 18px;"><b>{"Grand Total (Incl. Tax):" if is_us else "סה\"כ לתשלום כולל מיסים:"}</b> <span style="color: #facc15;">{total_with_tax:,.2f} {currency_sign}</span></p></div>'
    html += "</body></html>"
    return html

def reset_project_state():
    st.session_state["project_boq"] = {k: [] for k in disciplines_keys}
    keys_to_clear = ["elec_results", "elec_plan_raw", "elec_verified", "plumb_results", "plumb_plan_raw", "plumb_verified", "hvac_results", "hvac_plan_raw", "hvac_verified", "show_master_export"]
    for key in keys_to_clear: st.session_state.pop(key, None)

if "global_is_us" not in st.session_state: st.session_state["global_is_us"] = False
is_us_mode = st.session_state["global_is_us"]

disciplines_dict = {"elec": "Electrical & Lighting" if is_us_mode else "חשמל ומאור", "cons": "Construction (Walls)" if is_us_mode else "בניה (מחיצות ומעטפת)", "plum": "Plumbing & Gas" if is_us_mode else "אינסטלציה וגז", "tile": "Flooring & Tiling" if is_us_mode else "ריצוף וחיפוי", "hvac": "HVAC & Infrastructure" if is_us_mode else "מיזוג אוויר ותשתיות", "kitc": "Kitchen & Cabinetry" if is_us_mode else "מטבח ונגרות"}
disciplines_keys = list(disciplines_dict.keys())
disciplines_display = list(disciplines_dict.values())
tile_h = 2.40

if "project_boq" not in st.session_state: st.session_state["project_boq"] = {k: [] for k in disciplines_keys}
if "current_discipline" not in st.session_state: st.session_state["current_discipline"] = "elec"
if "show_master_export" not in st.session_state: st.session_state["show_master_export"] = False
if "saved_quotes" not in st.session_state: st.session_state["saved_quotes"] = []
if "app_mode" not in st.session_state: st.session_state["app_mode"] = None

# ========================================================
# מסך פתיחה - בחירת מודל
# ========================================================
if st.session_state["app_mode"] is None:
    ...
    if has_logo:
        ...
        with col_logo_cent[1]: st.image(LOGO_PATH, use_container_width=True)
    ...
    st.markdown(f"<h3 style='text-align: center; color: #64748b;'>{sub_ttl}</h3><br>", unsafe_allow_html=True)
    start_idx = 1 if is_us_mode else 0
    home_geo = st.selectbox("Choose Region & Language / בחירת אזור גיאוגרפי ושפה:", ["ישראל (שיטה מטרית | מחירון דקל | עברית) IL", "United States (Imperial - Feet & Inches | RSMeans | English) US"], index=start_idx)
    ...
    st.markdown(f"<p style='text-align: center; color: #475569; font-size: 18px; font-weight: 500;'>{choose_lbl}</p><br>", unsafe_allow_html=True)
    col_m1, col_m2 = st.columns(2, gap="large")
    with col_m1:
        tenant_btn_txt = ("Tenant Modifications (COs)\n\nFor Developers & General Contractors:\nCompares requested change drawings against baseline contract standards, precise delta calculations & structural safety shield." if is_us_mode else "מודל שינויי דיירים\n\nליזמים וקבלנים ראשיים:\nהשוואת שרטוט שינויים מול שרטוט מכר (סטנדרט). חישוב דלתא, מעקב מרחקי הזזה ובקרת מעטפת הנדסית.")
...
# ========================================================
# תפריט כלים עליון מורחב (Top Bar Tools & Progress)
# ========================================================
col_l, col_t = st.columns([1, 8])
with col_l:
    if st.button("S.A.Q Home", key="logo_btn_nav"):
        st.session_state["show_master_export"] = False
        st.rerun()
with col_t:
...
progress_html = "<div style='display: flex; justify-content: space-between; align-items: center; background: #e2e8f0; padding: 10px; border-radius: 8px; font-weight: 500; font-size: 14px; color: #1e293b; margin-bottom: 10px;'>"
for k, v in disciplines_dict.items():
    is_done = len(st.session_state["project_boq"].get(k, [])) > 0
    color = "#10b981" if is_done else "#94a3b8"
    icon = "<i class='fa-solid fa-check-circle'></i>" if is_done else "<i class='fa-regular fa-circle'></i>"
    progress_html += f"<span style='color: {color};'>{icon} {v}</span>"
progress_html += "</div>"
st.markdown(progress_html, unsafe_allow_html=True)

st.markdown("---")
t1, t2, t3, t4, t5, t6, t7 = st.columns([1.5, 1, 1, 1, 1, 1, 1])
with t1:
    scale_lbl = "Pixels per Foot (Scale 1:50)" if is_us_mode else "פיקסלים למטר (ברירת מחדל 1:50)"
    px_meter = st.number_input(scale_lbl, min_value=10.0, max_value=300.0, value=50.0, step=1.0)
with t2:
    with st.popover("<i class='fa-solid fa-calculator'></i>" if is_us_mode else "<i class='fa-solid fa-calculator'></i> מחשבון"):
        st.write("Unit Converter" if is_us_mode else "מחשבון עזר")
        sqm = st.number_input("SQM / מ\"ר", value=1.0)
        st.write(f"= {sqm * 10.7639:.2f} SQFT")
with t3:
    if st.button("<i class='fa-solid fa-rotate-right'></i>" if is_us_mode else "<i class='fa-solid fa-rotate-right'></i> איפוס"):
        st.session_state["project_boq"][curr_key] = []
        st.rerun()
with t4:
    if st.button("<i class='fa-solid fa-moon'></i>" if is_us_mode else "<i class='fa-solid fa-moon'></i> תצוגה"):
        st.session_state["dark_mode"] = not st.session_state["dark_mode"]
        st.rerun()
with t5:
    with st.popover("<i class='fa-solid fa-note-sticky'></i>" if is_us_mode else "<i class='fa-solid fa-note-sticky'></i> הערות"):
        st.text_area("Report Notes:" if is_us_mode else "הערות לדוח הסופי:")
with t6:
    if st.button("<i class='fa-solid fa-file-export'></i>" if is_us_mode else "<i class='fa-solid fa-file-export'></i> ייצוא"):
        st.session_state["show_master_export"] = True
        st.rerun()
with t7:
    inc_vat = st.checkbox("VAT (17%)" if is_us_mode else "מע\"מ (17%)", value=st.session_state.get("include_vat", True))
    st.session_state["include_vat"] = inc_vat
st.markdown("---")
...
with st.sidebar:
    ...
    if st.button("Restart Project" if is_us_mode else "חזרה להתחלה (איפוס)", use_container_width=True):
        st.session_state["app_mode"] = None
        reset_project_state()
        st.rerun()
    st.markdown("---")
    st.markdown("### Command Center" if is_us_mode else "### מרכז בקרה")
    if is_tenant:
        st.success(f"Active: Change Orders (COs)" if is_us_mode else f"פעיל: מודל שינויי דיירים")
    else:
        st.warning(f"Active: Renovation Contractors" if is_us_mode else f"פעיל: מודל קבלני שיפוצים")
    st.markdown("---")
    discipline_display = st.selectbox("Current Plan:" if is_us_mode else "סוג תוכנית לחישוב:", disciplines_display, index=curr_idx, key="disc_selector_widget", on_change=on_discipline_change)
    if curr_key == "tile":
        tile_h_def = 8.0 if is_us_mode else 2.40
        tile_h = st.number_input("Wet Area Wall Tiling Height (Feet):" if is_us_mode else "גובה חיפוי קירות רטובים (מטר):", min_value=5.0 if is_us_mode else 1.5, max_value=12.0 if is_us_mode else 3.5, value=tile_h_def, step=0.5 if is_us_mode else 0.10)
    st.markdown("---")
    st.markdown("### Saved Quotes" if is_us_mode else "### הצעות מחיר שמורות")
    history_key = "saved_quotes_tenant" if is_tenant else "saved_quotes_reno"
    ...
    with st.expander("View History" if is_us_mode else "צפה בהיסטוריה במודל זה", expanded=False):
        if not st.session_state[history_key]: st.info("No saved quotes for this model." if is_us_mode else "אין עדיין הצעות מחיר שמורות.")
        else:
    ...
    for d_key in disciplines_keys:
        ...
        with st.expander(f"{d_name} ({len(d_rows)} {item_lbl})", expanded=True):
            if d_rows:
                ...
                is_after_exec = st.session_state.get(f"{d_key}_after_exec", False)
                render_pricing_widget(d_rows, d_name, is_us=is_us_mode, is_after_exec=is_after_exec)
                for r in d_rows: all_project_rows.append((d_key, r))
            else: st.write("No quantities recorded in this plan yet." if is_us_mode else "טרם הופקו כמויות בתוכנית זו (0).")
    if all_project_rows:
        st.markdown("---")
        currency_sign = "$" if is_us_mode else "₪"
        tax_label = "Local Tax (8.5%)" if is_us_mode else "מע\"מ (17%)"
        tax_rate = 0.085 if is_us_mode else 0.17
        st.subheader(f"Comprehensive Financial Summary" if is_us_mode else f"סיכום תמחור פיננסי מלא")
        total_proj_pricing = 0
        for d_key, r in all_project_rows:
            desc = r.get("תיאור הפריט", "")
            qty = float(r.get("כמות מאושרת", 0))
            unit = r.get("יחידת מידה", "יח'")
            if is_us_mode:
            ...
            is_after_exec = st.session_state.get(f"{d_key}_after_exec", False)
            total_proj_pricing += qty * get_pricing_item_cost(desc, unit, is_us_mode, is_after_exec)
        if st.session_state.get("include_vat", True):
            proj_tax = total_proj_pricing * tax_rate
            proj_total_with_tax = total_proj_pricing + proj_tax
        else:
            proj_tax = 0.0; proj_total_with_tax = total_proj_pricing
        col_pr1, col_pr2, col_pr3 = st.columns(3)
        col_pr1.metric("Total Cost (Excl. Tax)" if is_us_mode else "סה\"כ עלות (ללא מע\"מ)", f"{total_proj_pricing:,.2f} {currency_sign}")
        if st.session_state.get("include_vat", True): col_pr2.metric(f"{tax_label}", f"{proj_tax:,.2f} {currency_sign}")
        col_pr3.metric("Grand Total" if is_us_mode else "סה\"כ לתשלום", f"{proj_total_with_tax:,.2f} {currency_sign}")
        ...
        with c_save:
            if st.button("Save, Finish & Start New" if is_us_mode else "שמור וסיים חישוב (התחל מחדש)", use_container_width=True):
                h_key = "saved_quotes_tenant" if is_tenant else "saved_quotes_reno"
                st.session_state[h_key].append({"date": time.strftime("%d/%m/%Y %H:%M"), "model": mode_lbl, "total": proj_total_with_tax, "currency": currency_sign, "boq_data": copy.deepcopy(st.session_state["project_boq"])})
                reset_project_state()
                st.rerun()
        with c_back:
        ...
        html_with_price = generate_master_export_html(st.session_state["project_boq"], title="Master BOQ + Pricing" if is_us_mode else "דוח כמויות ותמחור מקיף", mode_label=mode_lbl, is_us=is_us_mode, include_pricing=True)
        html_no_price = generate_master_export_html(st.session_state["project_boq"], title="Takeoff Quantities Only" if is_us_mode else "דוח כמויות נטו (ללא תמחור)", mode_label=mode_lbl, is_us=is_us_mode, include_pricing=False)
        col_ex1, col_ex2 = st.columns(2)
        with col_ex1:
            st.markdown("**With Pricing (BOQ + Estimate)**" if is_us_mode else "**דוח מלא (כמויות + תמחור הנדסי)**")
            st.download_button("Download Excel (XLS) w/ Prices" if is_us_mode else "הורד דוח Excel עם מחירים", data=html_with_price.encode("utf-8"), file_name=f"SAQ_Pricing_{mode_lbl}.xls", mime="application/vnd.ms-excel")
        with col_ex2:
            st.markdown("**Quantities Only (No Prices)**" if is_us_mode else "**דוח כמויות נטו (ללא מחירים)**")
            st.download_button("Download Excel (XLS) Qty Only" if is_us_mode else "הורד דוח Excel כמויות בלבד", data=html_no_price.encode("utf-8"), file_name=f"SAQ_Qty_{mode_lbl}.xls", mime="application/vnd.ms-excel")

# ========================================================
# עיבוד שרטוטים לפי תוכנית מרכזית
# ========================================================
else:
    if is_tenant:
        tenant_timing = st.radio("Execution Stage:" if is_us_mode else "שלב ביצוע (משפיע על התמחור הסופי):", ["Before Execution (Planning/Pricing)", "After Execution (Field Verification)"] if is_us_mode else ["לפני ביצוע (תכנון ותמחור מוקדם)", "אחרי ביצוע (בדיקת שטח ובקרה)"], horizontal=True)
        st.session_state[f"{curr_key}_after_exec"] = "After" in tenant_timing or "אחרי" in tenant_timing
    else: st.session_state[f"{curr_key}_after_exec"] = False
    ...
    if curr_key == "cons":
        ...
        with c_exec: f_plan = st.file_uploader("Proposed Change Plan (Required):" if is_us_mode else "שרטוט מוצע / ביצוע (חובה):", type=["pdf", "png", "jpg"], key="b_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline Standard (Optional):" if is_us_mode else "שרטוט מכר / מצב קיים (אופציונלי):", type=["pdf", "png", "jpg"], key="b_plan_std")
        with c_leg: f_leg = st.file_uploader("Legend (Optional):" if is_us_mode else "מקרא בניה (אופציונלי):", type=["pdf", "png", "jpg"], key="b_leg")
        st.markdown("---")
        b_wall_h = st.number_input(f"Partition Wall Height ({'Feet' if is_us_mode else 'מטר'}):" if is_us_mode else "גובה מחיצות פנים (מטר):", min_value=5.0 if is_us_mode else 1.5, max_value=15.0 if is_us_mode else 5.0, value=9.0 if is_us_mode else 2.70, step=0.5 if is_us_mode else 0.05)
        if f_plan:
            if st.button("Run Partition Takeoff" if is_us_mode else "הפעל חישוב כמויות בניה"):
                ...
                show_engineering_loader("S.A.Q AI scanning partitions...", is_us=is_us_mode)
                ...
                if is_tenant:
                    ...
                    else: st.success("Structural Safety Shield passed." if is_us_mode else "בקרת מעטפת הנדסית עברה בהצלחה (ללא פגיעה קונסטרוקטיבית).")
                ...
                if img_std is not None:
                    ...
                    diff_m = round(lin_exec - lin_std, 2); diff_sqm = round(diff_m * b_wall_h, 2)
                    if is_tenant:
                        st.subheader("Change Orders Delta Report" if is_us_mode else "דוח שינויים והפרשים נטו")
                        c1, c2, c3 = st.columns(3); unit_lbl = "FT" if is_us_mode else 'מ"א'
                        ...
                        c3.metric("Delta Net Difference:" if is_us_mode else "הפרש דלתא נטו:", f"{diff_m * (3.28084 if is_us_mode else 1):+.2f} {unit_lbl}")
                        b_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": abs(diff_m), "תיאור הפריט": f"Partition Walls Delta (Diff: {diff_m})" if is_us_mode else f"דלתא שינויי אורך קירות (הפרש: {diff_m})", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else 'מ"א'}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": abs(diff_sqm), "תיאור הפריט": f"Partition Area Delta (Height {b_wall_h})" if is_us_mode else f"דלתא שטח קירות (גובה {b_wall_h})", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}]
                    else:
                        ...
                        b_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": round(lin_std * b_wall_h, 2), "תיאור הפריט": f"Demolition of existing partitions" if is_us_mode else f"הריסת קירות פנים", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": round(lin_exec * b_wall_h, 2), "תיאור הפריט": f"New partition construction" if is_us_mode else f"בניית קירות חדשים", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}]
                    ...
                    b_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": lin_exec, "תיאור הפריט": "Net partition length" if is_us_mode else "אורך קירות פנים נטו", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else 'מ"א'}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": sqm_total, "תיאור הפריט": f"Partition area" if is_us_mode else f"שטח בניה קירות", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}]
                st.session_state["project_boq"][curr_key] = b_rows
                safe_render_table(b_rows, is_us=is_us_mode)
                render_pricing_widget(b_rows, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
                st.markdown("### Updated Proposed Plan" if is_us_mode else "### תוכנית מצב סופי מעובדת")
                st.image(cv2.cvtColor(disp_exec, cv2.COLOR_BGR2RGB), caption="Partition walls highlighted" if is_us_mode else "זיהוי אוטומטי של הקירות", use_container_width=True)
        else: st.info("Please input the construction plan." if is_us_mode else "אנא הזן את תוכנית הבניה.")
    ...
    # ----------------------------------------------------
        ...
        c_exec, c_std, c_leg = st.columns(3)
        with c_exec: f_plan = st.file_uploader("Proposed Plumbing Plan (Required):" if is_us_mode else "תוכנית אינסטלציה לביצוע (חובה):", type=["pdf", "png", "jpg"], key="p_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline Standard (Optional):" if is_us_mode else "תוכנית סטנדרט / קיים (אופציונלי):", type=["pdf", "png", "jpg"], key="p_plan_std")
        with c_leg: f_leg = st.file_uploader("Legend (Optional):" if is_us_mode else "מקרא אינסטלציה (אופציונלי):", type=["pdf", "png", "jpg"], key="p_leg")
        if f_plan:
            if st.button("Run Plumbing Takeoff & Verify" if is_us_mode else "הפעל ספירת אינסטלציה ואימות"):
                with st.spinner("Loading and validating blueprint..." if is_us_mode else "קורא ומאמת..."):
                    img_plan = load_raster(f_plan)
                    is_valid, v_msg = validate_drawing_discipline(img_plan, "plum", is_us=is_us_mode)
                if not is_valid: st.warning(v_msg + (" (Continuing anyway)" if is_us_mode else " (ממשיך בכל זאת)"))
                ...
                if img_std is not None:
                    ...
                    st.subheader("Plumbing Delta Report" if is_us_mode else "דוח אינסטלציה - שינויים ותשתיות")
                    ...
                    p_rows = []
                    total_pipe = 0.0; gas_points = 0
                    for idx, r in enumerate(relocs):
                        if "Gas" in r['type']: gas_points += 1
                        dist_disp = f"{r['distance_m'] * 3.28084:.2f} FT" if is_us_mode else f"{r['distance_m']} מ'"
                        total_pipe += r['distance_m']
                        p_rows.append({"מס'": idx + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": 1, "תיאור הפריט": f"Relocate {r['type']} (Shift {dist_disp})" if is_us_mode else f"הזזת {r['type']} (מרחק: {dist_disp})", "יחידת מידה": "Units" if is_us_mode else "יח'"})
                    for idx, a in enumerate(added):
                        if "Gas" in a['type']: gas_points += 1
                        total_pipe += 3.0
                        p_rows.append({"מס'": len(relocs) + idx + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": 1, "תיאור הפריט": f"New {a['type']} fixture added" if is_us_mode else f"הוספת נקודת {a['type']} חדשה", "יחידת מידה": "Units" if is_us_mode else "יח'"})
                    if total_pipe > 0: p_rows.append({"מס'": len(p_rows)+1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": round(total_pipe * (3.28084 if is_us_mode else 1), 2), "תיאור הפריט": "Concrete Piping to Nearest Drain" if is_us_mode else "ביטון צנרת מים וביוב בגין שינויים (עד לקולטן הקרוב)", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else "מ\"א"})
                    if gas_points > 0: p_rows.append({"מס'": len(p_rows)+1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": gas_points, "תיאור הפריט": "Gas Points (Changed/Added)" if is_us_mode else "נקודות גז - עבודת תשתית וצנרת גז", "יחידת מידה": "Units" if is_us_mode else "יח'"})
                    st.session_state["project_boq"][curr_key] = p_rows
                    st.warning("Please complete symbol verification to view final takeoff report." if is_us_mode else "אנא השלם את אימות הסמלים כדי לצפות בדוח הכמויות.")
                    st.stop()
                else:
                    ...
                    if len(yellows) < 6:
                        needed = 6 - len(yellows); base_idx = len(formatted_results) + 1
                        for i in range(needed):
        ...
        if "plumb_results" in st.session_state:
            res = st.session_state["plumb_results"]; raw_plan = st.session_state["plumb_plan_raw"]
            ...
            render_pricing_widget(rows_p, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
        ...
        elif not f_plan: st.info("Please input the plumbing plan." if is_us_mode else "אנא הזן את תוכנית האינסטלציה.")
    ...
    # ----------------------------------------------------
        ...
        with c_exec: f_plan = st.file_uploader("Proposed Flooring Plan (Required):" if is_us_mode else "תוכנית ריצוף מוצעת (חובה):", type=["pdf", "png", "jpg"], key="f_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline Flooring Standard (Optional):" if is_us_mode else "תוכנית סטנדרט קבלן (אופציונלי):", type=["pdf", "png", "jpg"], key="f_plan_std")
        if f_plan:
            if st.button("Run Flooring Takeoff" if is_us_mode else "הפעל חישוב שטחי ריצוף וחיפוי"):
                with st.spinner("Loading blueprint..." if is_us_mode else "קורא שרטוט..."): img_plan = load_raster(f_plan)
                ...
                if img_std is not None:
                    ...
                    f_std_sqm, _, w_std_sqm, _ = calc_flooring_and_wall_tiling(img_std, tile_h, px_meter, [f["center"] for f in fixtures_std])
                    diff_floor = round(floor_sqm - f_std_sqm, 2); diff_wall = round(wet_wall_sqm - w_std_sqm, 2)
                    ...
                    c2.metric("Wet Wall Cladding Delta:" if is_us_mode else "הפרש חיפוי קירות:", f"{wet_wall_sqm * (10.7639 if is_us_mode else 1):.2f} {'SQFT' if is_us_mode else 'מ\"ר'}", f"{diff_wall * (10.7639 if is_us_mode else 1):+.2f} Delta")
                    f_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": abs(diff_floor), "תיאור הפריט": f"Net Flooring (Delta: {diff_floor})" if is_us_mode else f"תוספת שטח ריצוף נטו (דלתא)", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": abs(diff_wall), "תיאור הפריט": f"Wet Room Wall Tiling (Delta: {diff_wall})" if is_us_mode else f"תוספת שטח חיפוי קירות (דלתא)", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}]
                else:
                    ...
                    f_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": floor_sqm, "תיאור הפריט": "Net flooring area" if is_us_mode else "שטח ריצוף כללי נטו", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": wet_wall_sqm, "תיאור הפריט": f"Wet room wall cladding (Height {tile_h})" if is_us_mode else f"שטח חיפוי קירות רטובים", "יחידת מידה": "Square Feet (SQFT)" if is_us_mode else 'מ"ר'}]
                st.session_state["project_boq"][curr_key] = f_rows
                safe_render_table(f_rows, is_us=is_us_mode)
                render_pricing_widget(f_rows, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
        ...
        else: st.info("Please input the flooring plan." if is_us_mode else "אנא הזן את תוכנית הריצוף.")
    ...
    # ----------------------------------------------------
        ...
        c_exec, c_std, c_leg = st.columns(3)
        with c_exec: f_plan = st.file_uploader("Proposed Electrical Plan (Required):" if is_us_mode else "תוכנית חשמל מוצעת (חובה):", type=["pdf", "png", "jpg"], key="e_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline Standard (Optional):" if is_us_mode else "תוכנית סטנדרט / מצב קיים (אופציונלי):", type=["pdf", "png", "jpg"], key="e_plan_std")
        with c_leg: f_leg = st.file_uploader("Legend (Optional):" if is_us_mode else "מקרא חשמל ומאור (אופציונלי):", type=["pdf", "png", "jpg"], key="e_leg")
        if f_plan:
            if st.button("Run Electrical Takeoff & AI Verify" if is_us_mode else "הפעל פענוח חשמל ואימות AI"):
                with st.spinner("Loading and validating..." if is_us_mode else "קורא ומאמת..."):
                    img_plan = load_raster(f_plan)
                    is_valid, v_msg = validate_drawing_discipline(img_plan, "elec", is_us=is_us_mode)
                if not is_valid: st.warning(v_msg + (" (Continuing anyway)" if is_us_mode else " (ממשיך בכל זאת)"))
                st.session_state["elec_verified"] = False
                show_engineering_loader("S.A.Q AI analyzing outlets...", is_us=is_us_mode)
                if f_std:
                    st.warning("Please complete symbol verification to view final takeoff report." if is_us_mode else "מצב השוואה פעיל. אנא השלם את אימות הסמלים במערכת כדי לצפות בדוח הכמויות.")
                    st.stop()
        ...
        if "elec_results" in st.session_state:
            res = st.session_state["elec_results"]; raw_plan = st.session_state["elec_plan_raw"]
            ...
            render_pricing_widget(rows_e, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
        ...
        elif not f_plan: st.info("Please input the electrical plan." if is_us_mode else "אנא הזן את תוכנית החשמל.")

    # ----------------------------------------------------
    # 5. מודול מיזוג אוויר ותשתיות 
    # ----------------------------------------------------
    elif curr_key == "hvac":
        c_exec, c_std, c_leg = st.columns(3)
        with c_exec: f_plan = st.file_uploader("Proposed HVAC Plan (Required):" if is_us_mode else "תוכנית מיזוג מוצעת (חובה):", type=["pdf", "png", "jpg"], key="h_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline HVAC Plan (Optional):" if is_us_mode else "תוכנית מיזוג סטנדרט / קיים (אופציונלי):", type=["pdf", "png", "jpg"], key="h_plan_std")
        with c_leg: f_leg = st.file_uploader("HVAC Legend (Optional):" if is_us_mode else "מקרא מיזוג וקחז\"מ (אופציונלי):", type=["pdf", "png", "jpg"], key="h_leg")
        if f_plan:
            if st.button("Run HVAC Takeoff & AI Verify" if is_us_mode else "הפעל סריקת מיזוג אוויר ותשתיות"):
                with st.spinner("Loading and validating..." if is_us_mode else "קורא ומאמת..."):
                    img_plan = load_raster(f_plan)
                    is_valid, v_msg = validate_drawing_discipline(img_plan, "hvac", is_us=is_us_mode)
                if not is_valid: st.warning(v_msg + (" (Continuing anyway)" if is_us_mode else " (ממשיך בכל זאת)"))
                st.session_state["hvac_verified"] = False
                show_engineering_loader("S.A.Q AI scanning AC units...", is_us=is_us_mode)
                img_std = load_raster(f_std) if f_std else None
                if img_std is not None:
                    ...
                    st.metric("Added Piping/Infrastructure:" if is_us_mode else "תוספת צנרת ותשתיות:", f"{extra_pipe * 3.28084:.2f} FT" if is_us_mode else f"{extra_pipe:.2f} מ' רץ")
                    h_rows = []
                    for idx, r in enumerate(relocs):
                        dist_disp = f"{r['distance_m'] * 3.28084:.2f} FT" if is_us_mode else f"{r['distance_m']} מ'"
                        h_rows.append({"מס'": idx + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": 1, "תיאור הפריט": f"Relocate {r['type']} (Shift {dist_disp})" if is_us_mode else f"הזזת {r['type']} (מרחק: {dist_disp})", "יחידת מידה": "Units" if is_us_mode else "יח'"})
                    for idx, a in enumerate(added):
                        h_rows.append({"מס'": len(relocs) + idx + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": 1, "תיאור הפריט": f"New {a['type']} added" if is_us_mode else f"תוספת {a['type']} חדשה", "יחידת מידה": "Units" if is_us_mode else "יח'"})
                    if extra_pipe > 0:
                        wall_chase = min(extra_pipe, len(added) * 2.5 + len(relocs) * 1.5)
                        floor_conc = max(0, extra_pipe - wall_chase)
                        if wall_chase > 0: h_rows.append({"מס'": len(h_rows) + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": round(wall_chase * (3.28084 if is_us_mode else 1), 2), "תיאור הפריט": "Wall Chasing for AC Pipe" if is_us_mode else "חציבת צנרת מיזוג בקיר", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else "מ\"א"})
                        if floor_conc > 0: h_rows.append({"מס'": len(h_rows) + 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": round(floor_conc * (3.28084 if is_us_mode else 1), 2), "תיאור הפריט": "Floor Concrete for AC Pipe" if is_us_mode else "ביטון צנרת מיזוג ברצפה", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else "מ\"א"})
                    st.session_state["project_boq"][curr_key] = h_rows
                    st.warning("Please complete symbol verification to view final takeoff report." if is_us_mode else "אנא השלם את אימות הסמלים כדי לצפות בדוח.")
                    st.stop()
                else:
                    plan_gray = cv2.cvtColor(img_plan, cv2.COLOR_BGR2GRAY)
                    _, plan_inv = cv2.threshold(plan_gray, 230, 255, cv2.THRESH_BINARY_INV)
        ...
        if "hvac_results" in st.session_state:
            res = st.session_state["hvac_results"]; raw_plan = st.session_state["hvac_plan_raw"]
            ...
            render_pricing_widget(rows_h, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
        ...
        elif not f_plan: st.info("Please input the HVAC plan." if is_us_mode else "אנא הזן את תוכנית המיזוג.")
    ...
    # ----------------------------------------------------
        ...
        c_exec, c_std, c_leg = st.columns(3)
        with c_exec: f_plan = st.file_uploader("Proposed Kitchen Plan (Required):" if is_us_mode else "תוכנית מטבח מוצעת (חובה):", type=["pdf", "png", "jpg"], key="k_plan_exec")
        with c_std: f_std = st.file_uploader("Baseline Standard (Optional):" if is_us_mode else "תוכנית סטנדרט / קיים (אופציונלי):", type=["pdf", "png", "jpg"], key="k_plan_std")
        with c_leg: f_leg = st.file_uploader("Legend (Optional):" if is_us_mode else "מקרא נגרות (אופציונלי):", type=["pdf", "png", "jpg"], key="k_leg")
        if f_plan:
            if st.button("Run Kitchen Takeoff" if is_us_mode else "הפעל סריקת תוכנית מטבח"):
                with st.spinner("Loading and validating blueprint..." if is_us_mode else "קורא ומאמת..."):
                    img_plan = load_raster(f_plan)
                    is_valid, v_msg = validate_drawing_discipline(img_plan, "kitc", is_us=is_us_mode) 
                if not is_valid: st.warning(v_msg + (" (Continuing anyway)" if is_us_mode else " (ממשיך בכל זאת)"))
                show_engineering_loader("S.A.Q AI scanning cabinetry...", is_us=is_us_mode)
                lin_cab, disp_img = calc_kitchen_cabinetry(img_plan, px_meter)
                k_rows = [{"מס'": 1, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": lin_cab, "תיאור הפריט": "Lower Kitchen Cabinets" if is_us_mode else "ארונות מטבח תחתונים (מטר רץ)", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else "מ\"א"}, {"מס'": 2, "תמונת סמל": "", "image_uri": "", "כמות מאושרת": lin_cab, "תיאור הפריט": "Stone Countertop" if is_us_mode else "משטח שיש למטבח", "יחידת מידה": "Linear Feet (FT)" if is_us_mode else "מ\"א"}]
                st.session_state["project_boq"][curr_key] = k_rows
                safe_render_table(k_rows, is_us=is_us_mode)
                render_pricing_widget(k_rows, disciplines_dict[curr_key], is_us=is_us_mode, is_after_exec=st.session_state.get(f"{curr_key}_after_exec", False))
                st.image(cv2.cvtColor(disp_img, cv2.COLOR_BGR2RGB), caption="Kitchen Layout" if is_us_mode else "מבט פריסת מטבח", use_container_width=True)
        else: st.info("Please input the kitchen plan." if is_us_mode else "אנא הזן את תוכנית המטבח.")

    # ========================================================
    # כרטיסיות מעבר מהיר בין תוכניות בתחתית הדף
    # ========================================================
    st.markdown("---")
    st.write("**Quick Navigation:**" if is_us_mode else "**מעבר מהיר לתוכנית נוספת:**")
    rem_keys = [k for k in disciplines_keys if k != curr_key]
    cols_nav = st.columns(len(rem_keys))
    for i, d_target in enumerate(rem_keys):
        if cols_nav[i].button(disciplines_dict[d_target], key=f"btn_nav_{d_target}", use_container_width=True):
            set_discipline_programmatically(d_target)

    st.markdown("<br>", unsafe_allow_html=True)
    c_fin, _, _ = st.columns([1, 1, 1])
    with c_fin:
        if st.button("Finalize & Export Reports" if is_us_mode else "סיום הפרויקט והפקת דוחות סופיים", key=f"btn_finish_master_{curr_key}", use_container_width=True, type="primary"):
            st.session_state["show_master_export"] = True
            st.rerun()
